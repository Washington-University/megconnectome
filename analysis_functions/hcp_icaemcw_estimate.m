...
emcw_order=net_seeds.emcw_order;
...
ntp=size(source.power,2);
nwin=fix((ntp-window2)/step2);

disp('estimating seed correlations')
for iemcw=1:size(emcw_order,1)
for k=1:nwin
    iref(1:3)=net_seeds.cortex_index(emcw_order(iemcw,:));
    ...
    r=corrcoef(ref(1,vect1(tmp)),ref(2,vect1(tmp))); c_p(1,k)=r(1,2);
    r=corrcoef(ref(1,vect1(tmp)),ref(3,vect1(tmp))); c_p(2,k)=r(1,2);
    r=corrcoef(ref(1,vect1(tmp)),ref(4,vect1(tmp))); c_p(3,k)=r(1,2);
end
    c_sn(iemcw,:)=min(abs(c_p(1:2,:)));
    c_out(iemcw,:)=c_p(3,:);
end
...
%     plot(time_corr,c_sn(1,:),'b'); hold on;
%     plot(time_corr,c_sn(2,:),'r')
%     plot(time_corr,c_sn(3,:),'k')
...
if isempty(iii)==0
    ...
    iref_c=net_seeds.cortex_index;
    ref1=source.power(iref_c(1),:);
    ...
    connect_mcw.seeds_indx=net_seeds.cortex_index;
