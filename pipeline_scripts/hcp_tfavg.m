...
if ~exist('subjectid', 'var')
    subjectid1 = tokF1{end-7};
    if ~isempty(filename2),
        subjectid2 = tokF2{end-7};
...
if ~exist('experimentid', 'var')
    experimentid1 = tokF1{end-5};
    if ~isempty(filename2),
        experimentid2 = tokF2{end-5};
        if ~strcmp(experimentid1,experimentid2),
            error('the two filenames provided have the same subjectid DUT different experimentid');
        else
...
scanmnem=scanmnem1;
%-----
if ~exist('avgmode', 'var')
    avgmode={'mag','planar'}; % or planar
end
if ~iscell(avgmode)
    avgmode={avgmode};
end
...
[indxPipe,ind2]=match_str(pipenames,'tfavg');

if isempty(indxPipe)
    error('No tfavg constasts were found in the allcontrast list');
else
...
Ntfavgcontr=length(curPipecntrList{1});
...
for iG=1:length(procGroups),
    for iBand=1:size(bandDef,1)
        hcp_check_pipelineoutput('tmegpreproc', 'subject', subjectid, 'experiment', experimentid, 'scan', scanid1,'datagroup',procGroups{iG},'band',bandDef(:,1));
        if Nfiles==2,
            hcp_check_pipelineoutput('tmegpreproc', 'subject', subjectid, 'experiment', experimentid, 'scan', scanid2,'datagroup',procGroups{iG},'band',bandDef(:,1));
        end
    end
end
...
for iMode=1:length(avgmode)
    
    cfg = [];
    cfg.avgmode        =  avgmode{iMode}; %'mag','planar'
    ...
    [outStatus] = hcp_tfavg_contrasts(cfg);
    
end

hcp_check_pipelineoutput('tfavg', 'subject', subjectid, 'experiment', experimentid, 'scan', scanmnem,'avgmode',avgmode,'contrasts',procCntrNames);
